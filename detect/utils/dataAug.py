...
def random_rotation90_flip(img, bboxes, p=0.5):
    rotate_bboxes = bboxes.copy()
    if random.random() < p:
        _, width, _ = img.shape
        img = np.rot90(img)
        rotate_bboxes[:, [0, 2]] = bboxes[:, [1, 3]]
        rotate_bboxes[:, [1, 3]] = width - bboxes[:, [2, 0]]
    return img, rotate_bboxes